import pandas as pd

from analytics import headache_sleep_pattern


df = pd.read_csv(
    "data/health_data.csv"
)

result = headache_sleep_pattern(df)


print("\n========== PERSONAL PATTERN DISCOVERY ==========\n")

print(
    f"Total headache events: "
    f"{result['total_headaches']}"
)

print(
    f"Headaches after below-average sleep: "
    f"{result['below_average_sleep']}"
)

print(
    f"Percentage: "
    f"{result['percentage']}%"
)

print("\nLifePrint learned:")

print(
    f"{result['percentage']}% of your recorded "
    f"headaches occurred after below-average sleep."
)